...
このアプリはサーバーを持たず、データは**各ユーザーの端末内（ブラウザの IndexedDB）にのみ**保存されます。公開時は以下を理解・周知してください。

### データの保存先とプライバシー
- 各ユーザーのレシピ・リスト・画像は、**その人の端末・ブラウザの中だけ**に保存されます。他人のデータが見えることはありません。
- **同期はされません**。同じ人でも端末・ブラウザが違えば別データです（PC と スマホ、Safari と Chrome は別）。
- アカウント不要。公開 URL にアクセスすると、各自が空の「自分専用インスタンス」を得ます。
- 公開リポジトリ・デプロイ成果物に**個人データは含まれません**（本番ビルドではサンプルデータを投入しません。サンプルはローカル開発時のみ）。

### ⚠ データ消失を防ぐ（最重要）
- **iOS Safari は、ホーム画面に追加していないサイトの保存データを約7日間アクセスが無いと消去します。** たまにブラウザで開くだけだとレシピが消えることがあります。
- 対策として、起動時に `navigator.storage.persist()` で**永続ストレージを要求**しています（ベストエフォート）。確実にするには、ユーザーに **「ホーム画面に追加」して PWA として使ってもらう**ことを推奨してください（アプリ内にも案内を表示）。
- 永続化の状態と使用量は **設定 → ストレージ**で確認できます。

### バックアップは各自の責任
- データは端末ローカルのみのため、**端末故障・ブラウザのデータ消去・アンインストールで失われます**。
- **設定 → バックアップ**から ZIP を定期的にエクスポートするよう促してください（最終バックアップから7日経過でリマインドが出ます）。
- **インポートは既存データを全置換します**。復元時は注意。

### 将来のアップデート時
- データモデルを変更する場合は、**Dexie のバージョンを上げてマイグレーションを実装**してください（現在 `schemaVersion` は 1）。これを怠ると既存ユーザーのデータが壊れる可能性があります。
